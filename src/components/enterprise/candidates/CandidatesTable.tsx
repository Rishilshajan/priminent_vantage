                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-[#f7f6f8] dark:bg-[#130d1a]/50 border-b border-primary/5">
                            <th className="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-slate-400">Candidate</th>
                            <th className="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-slate-400">Simulation</th>
                            <th className="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-slate-400">Progress</th>
                            <th className="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-slate-400 text-center">Score</th>
                            <th className="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-slate-400">Skills Demonstrated</th>
                            <th className="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-slate-400">Date Joined</th>
                    ...
                    <tbody className="divide-y divide-primary/5">
                        {candidates.map((candidate) => (
                            <tr key={candidate.id} className="hover:bg-primary/5 transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        {candidate.avatar ? (
                                            ...
                                            <div className="w-9 h-9 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center font-bold text-slate-400 text-xs">
                                                {candidate.initials}
                                            </div>
                                        )}
                                        <div>
                                            <p className="text-sm font-bold text-[#140d1b] dark:text-white">{candidate.name}</p>
                                            <p className="text-[11px] text-slate-500">{candidate.email}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                    <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{candidate.simulation}</span>
                                </td>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3 min-w-[120px]">
                                        ...
                                        <span className="text-xs font-bold text-slate-600 dark:text-slate-300">{candidate.progress}%</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-center">
                                    {candidate.status === 'completed' && candidate.score ? (
                                        <span className={`px-2.5 py-1 text-xs font-bold rounded-lg shadow-sm ${candidate.score >= 90 ? 'bg-primary text-white' : 'bg-primary/10 text-primary border border-primary/20'}`}>
                                            {candidate.score}/100
                                        </span>
                                    ) : (
                                        <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-400 text-xs font-bold rounded-lg">Pending</span>
                                    )}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="flex flex-wrap gap-1">
                                        {candidate.skills.map((skill) => (
                                            <span key={skill} className="text-[10px] px-2 py-0.5 bg-primary/5 border border-primary/10 text-primary font-bold rounded">
                                                {skill}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <td className="px-6 py-4">
                                    <span className="text-xs text-slate-500">{candidate.dateJoined}</span>
                                </td>
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <Button variant="ghost" className="h-8 px-3 py-1 text-xs font-bold text-primary bg-primary/10 rounded hover:bg-primary transition-all hover:text-white">
                                            View Profile
                                        </Button>
                                    </div>
                                </td>
