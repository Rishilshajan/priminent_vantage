  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-[32px] border border-slate-100 bg-white shadow-xl shadow-slate-200/50 transition-all hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10 dark:border-white/5 dark:bg-[#1e1429] dark:shadow-none">
      ...
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        <div className="absolute -bottom-5 right-6 z-10 size-12 overflow-hidden rounded-xl border-4 border-white bg-white shadow-xl dark:border-[#1e1429] dark:bg-[#1e1429] lg:-bottom-6 lg:right-8 lg:size-16 lg:rounded-2xl">
          <Image
      ...
      <div className="flex flex-1 flex-col p-6 pt-8 lg:p-8 lg:pt-10">
        <div
          className="mb-3 flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.2em] text-primary lg:mb-4 lg:text-[10px]"
        ...
        <div className="mb-6 flex flex-wrap gap-2 lg:mb-8">
          {simulation.simulation_skills
            ?.slice(0, 2)
            .map((skill: any, idx: number) => (
              <span
                key={idx}
                className="flex items-center gap-1 rounded-lg bg-slate-50 px-2.5 py-1 text-[10px] font-bold text-slate-600 dark:bg-white/5 dark:text-slate-400 lg:gap-1.5 lg:px-3 lg:py-1.5 lg:text-[11px]"
              >
        ...
        {showProgress && (
          <div className="mb-6 space-y-2 lg:mb-8">
            <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-slate-400 lg:text-[11px]">
              <span>Progress</span>
              <span className="text-primary" style={brandColorText}>
                {progress}%
              </span>
            </div>
        ...
        <div className="flex items-center gap-3 lg:gap-4">
          {/* Duration */}
          <div className="inline-flex items-center gap-2 rounded-2xl border border-slate-200/70 dark:border-white/10 bg-slate-50/70 dark:bg-white/5 px-3 py-2">
            <div className="size-9 rounded-xl bg-white/70 dark:bg-white/5 border border-slate-200/70 dark:border-white/10 grid place-items-center">
              <Clock size={14} className="text-slate-700 dark:text-slate-200" />
            </div>
        ...
        {/* Action Footer */}
        <div className="mt-auto flex items-center justify-end pt-5 border-t border-slate-50 dark:border-white/5 lg:pt-6">
          <Link
            href={`/student/simulations/${simulation.id}/preview`}
            className="flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-slate-900 text-white shadow-lg transition-all hover:scale-110 hover:bg-primary dark:bg-white/10 dark:hover:bg-primary"
